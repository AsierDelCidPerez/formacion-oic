...
void aleatorioGrafo(bool ponderado){
    ...
    for(int i=0;i<g.size();i++)
        for(auto j : g[i]) {
            int num = generarNumero(1,3);
            cout << i+1 << " " << j+1 << (ponderado ? (" " + to_string(generarNumero(1,faktor))) : "") << " ";
            if(num == 1) cout << "grava\n";
            else if(num == 2) cout << "empedrado\n";
            else cout << "vegetacion\n";
        }
...
void generatePatindrmo(int T){
    // cout << T << "\n";
    while(T--){
        cout << generateFromBank("ABCDEFGHIJKLMNOPQRSTUVWXYZ", generarNumero(10, 1000)) << "\n";
    }
}

void generateCometas(int T){
    while(T--){
        int N = generarNumero(1,100),M=generarNumero(1,1000);
        cout << N << " " << M << "\n";
        while(N--){
            cout << generarNumero(1,1000) << " " << generarNumero(1,1000) << "\n";
        }
    }
}

void generateValdecilla(int T){
    cout << T << "\n";
    while(T--){
        for(int i=0;i<5;i++){
            for(int j=0;j<5;j++){
                cout << generarNumero(-500, 500) << " ";
            }
            cout << "\n";
        }
        ...
        cout << N << "\n";
        while(N--){
            cout << generateFromBank("ATGC", generarNumero(1,1000)) << "\n";
        }
    }
}


void generateDesgraciado(int T){
    while(T--){
        int N = generarNumero(1,100);
        cout << N << "\n";
        while(N--){
            cout << generarNumero(1,1000000) << " ";
        }
        cout << "\n";
    }
    cout << "0\n";
...
void generateCambio(int T){
    ...
    while(T--){
        ...
        cout << "\n";
        cout << generarNumero(1,10000) << "\n";
    }
}

void generateCazatesoros(int T){
    while(T--){
        int t = generarNumero(1,1000);
        cout << t << "\n";
        int N = generarNumero(1,200);
        cout << N << "\n";
        while(N--){
            cout << generarNumero(1,500) << " " << generarNumero(1,10000) << "\n";
        }
    }
}

void generateVacasPensantes(int T){
    while(T--){
        int N = generarNumero(1,100);
        cout << N << "\n";
        while(N--){
...
void generateRomanticismo(int T){
    while(T--){
        int N = generarNumero(1, 1000);
        cout << N << "\n";
        while(N--){
            printf("%02d:%02d %i\n", generarNumero(0, 21),generarNumero(0, 59), generarNumero(1, 179));
        }
    }
    cout << "0\n";
}

void generateAliBaba(int T){
    cout << T << "\n";
    while(T--){
        int N = generarNumero(1,100);
        int M = generarNumero(1,1000);
        cout << N << " " << M << "\n";
        while(N--){
            cout << generarNumero(1,100) << " " << generarNumero(1,100) << "\n";
        }
    }
}

vector<int> getFormatted(int C){
    int mins = C/60;
    C%=60;
    int hours = mins/60;
    mins %=60;
    vector<int> res = {hours, mins, C};
    return res;
}

void generateTrenesHorarios(int T){
    while(T--){
        int N=generarNumero(1,100),M=generarNumero(1,100);
        cout << N << " " << M<< "\n";
        int anterior = 1;
        vector<int> usados;
        while(N--){
            int C = generarNumero(anterior+1, min(89999, anterior+1000));
            vector<int> hora = getFormatted(C);
            usados.push_back(C);
            printf("%02d:%02d:%02d ", hora[0],hora[1], hora[2]);

            anterior = C;
        }
        cout << "\n";
        while(M--){
            int C = generarNumero(1,89999);
            vector<int> hora = getFormatted(C);
...
void generateRutaPerfecta(int T){
    while(T--){
        aleatorioGrafo(true);
    }
}

void generateCofreTeledirigido(int T){
    while(T--){
        int N = generarNumero(1,1000);
        cout << N << "\n";
        for(int i=0;i<N;++i){
...
void generateCarpintero(int T){
    while(T--){
        int L = generarNumero(10,10000);
        int N = generarNumero(1,10);
        cout << L << " " << N << "\n";
        while(N--){
            cout << generarNumero(1,L/4) << " ";
        }
        cout << "\n";
    }
    cout << "0 0\n";
...
void generateAmazon(int T){
    while(T--){
        int cantidad=generarNumero(1,10000),n=generarNumero(1,50);
        cout << cantidad << " " << n << "\n";
        while(n--){
            cout << generarNumero(1,500) << " ";
        }
        cout << "\n";
    }
}

void potenciometroGenerar(int T){
    while(T--){
        int N = generarNumero(1,1000);
        cout << N << "\n";
        for(int i=0;i<N;++i){
        ...
        int k = generarNumero(1,10000);
        while(k--){
            int al = generarNumero(0,1);
            if(al > 0){
                cout << "S " << generarNumero(1,N) << " " << generarNumero(1,10000) << "\n";
            }else{
                int inicio = generarNumero(1,N-1);
                int fin = generarNumero(inicio, N);
                cout << "M " << inicio << " " << fin << "\n";
            }
        }
        cout << "FIN\n";
...
void productosGenerar(int T){
    while(T--){
        int N = generarNumero(1,10000);
        cout << N << "\n";
        for(int i=0;i<N;++i){
            cout << generarNumero(-100,100) << " ";
        }
        int k = generarNumero(1,10000);
        while(k--){
            int al = generarNumero(0,1);
            if(al > 0){
                cout << "C " << generarNumero(1,N) << " " << generarNumero(-100,100) << "\n";
            }else{
                int inicio = generarNumero(1,N-1);
                int fin = generarNumero(inicio, N);
                cout << "P " << inicio << " " << fin << "\n";
